...
function Product({setCartbadge}) {
  ...
  let product;
  if (!encodedData){  product = "";} //this should never be the case ... 
  else { product = JSON.parse(atob(encodedData));}
    ...
    <div className="min-h-screen">
      ...
      <ScrollArea className="my-16 w-6/12 max-h-screen whitespace-nowrap">
        ...
        <div className='productinfo flex flex-col my-16 mr-60 gap-12'>
          ...
          <h1 className='text-6xl leading-normal'>{product.title}</h1>
          <span className='text-3xl'>Rs. {product.price.toLocaleString()} 
          {product.is_out_stock ?  <span className='ml-8 bg-slate-400 px-8 py-2 rounded-full cursor-pointer'> Sold Out </span> : <span></span> }
          </span>
